...
Description: Random tests for the village card
*/
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < iterations; i++)
    {
        ...
        int k[10] = {
            village, -1, -1, -1, -1, -1, -1, -1, -1, -1};
        int j;
        for (j = 1; j < 10; j++)
        {
            int loop = 1;

            // Add a new card, make sure it is not already included in the array
            while (loop)
            {
                int temp = rand() % 27;
        ...
        // Base conditions: set player, turn and phase; also clear hand, deck, discard
        state.whoseTurn = 0;
        state.phase = 0;
        state.handCount[0] = 0;
        ...
        // Fill hand with up to 19 more random cards
        for (j = 0; j < rand() % 20; j++)
        {
            state.handCount[0]++;

            // Add one of 17 valid cards
            int temp = rand() % 17;
            if (temp < 10)
            {
        ...
        // Not testing 0 because 0 is not playing village - it is testing playCard instead
        state.numActions = rand() % 3 + 1;

        // Make sure coins variable reflects right value
        ...
        if (discardCount + deckCount == 0)
        {
            ...
            // Draw one cards, then subtract the one played card
            if (handCount != state.handCount[0])
            {
                {
                    printf("%d - ERROR: wrong number of cards drawn to hand\n", i + 1);
                }
        ...
        // Check to see if playedCardCount is correct
