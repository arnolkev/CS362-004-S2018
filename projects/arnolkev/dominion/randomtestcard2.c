...
int main() {
    ...
    for(i = 0; i < 500; i++){
        ...
        int ogaction = state->numActions;
        cardEffect(village, 0, 0, 0, state, 0, 0);
        assertTrue(oghandcount, state->handCount[0]);
        assertTrue(deckdiscard-1, (state->deckCount[0] + state->discardCount[0]));
        assertTrue(ogaction+2, state->numActions );
        free(state);
    }
     printf("Status: %d failures of the playVillage\n", failure);
    return 0;
    }
